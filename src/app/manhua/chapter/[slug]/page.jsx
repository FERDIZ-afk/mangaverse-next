...
export default function ChapterPage({ params }) {
  ...
  const recordReadingHistory = async (chapterData) => {
    try {
      ...
      if (!chapterData || !chapterData.manhua_param) {
        console.error("Data chapter tidak valid untuk riwayat baca");
        return;
      }

      const manhuaTitle = chapterData.title
      ...
      console.log("Mencatat riwayat baca untuk:", {
        manhuaTitle,
        chapterSlug: slug,
        manhuaParam: chapterData.manhua_param,
      });
      ...
      const response = await fetch("/api/history", {
        ...
        body: JSON.stringify({
          manhuaSlug: chapterData.manhua_param,
          manhuaTitle: manhuaTitle,
          chapterSlug: slug,
      ...
      if (result.success && result.data) {
        try {
          ...
          let historyData = [];
          try {
            const localHistory = localStorage.getItem("manhuaReadingHistory");
          ...
          // Hapus entri yang sama jika sudah ada (berdasarkan manhuaSlug dan chapterSlug)
          const oldLength = historyData.length;
          historyData = historyData.filter(
            (item) =>
              !(
                item.manhuaSlug === result.data.manhuaSlug &&
          ...
          localStorage.setItem("manhuaReadingHistory", dataToSave);

          // Verifikasi data tersimpan
          const savedData = localStorage.getItem("manhuaReadingHistory");
          if (savedData) {
            console.log(
            ...
            sessionStorage.setItem("manhuaReadingHistory_backup", dataToSave);
          } else {
  ...
  const generatePlaceholderImageUrl = (index, title, chapterNumber) => {
    ...
    const svgContent = `<svg width="800" height="1200" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="400" y="1000" font-family="Arial" font-size="18" fill="#FFFFFF" text-anchor="middle">manhuaVerse Fallback Image</text>
    </svg>`;
  ...
  const manhuaTitle = chapter.title.split(" Chapter")[0];
            ...
            <div className="flex flex-wrap md:flex-nowrap justify-between items-center gap-4">
              ...
              <div className="flex space-x-3 w-full md:w-auto">
                {chapter.prev_chapter && (
                  <Link
                    href={`/manhua/chapter/${chapter.prev_chapter}`}
                ...
                {chapter.next_chapter && (
                  <Link
                    href={`/manhua/chapter/${chapter.next_chapter}`}
          ...
          <div className="flex flex-wrap md:flex-nowrap justify-between items-center gap-4">
            ...
            {chapter.next_chapter ? (
              ...
              <div className="ml-auto flex-1 md:flex-auto text-right">
                <Link
                  href={`/manhua/${chapter.manhua_param}`}
